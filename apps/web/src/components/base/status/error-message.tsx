export function humanizeError(error: unknown, fallback = "Something went wrong. Please try again.") {
  ...
  if (text.includes("rpc") || text.includes("fetch") || text.includes("network")) return "Base Sepolia RPC is temporarily unavailable.";
  return raw || fallback;
}

export function ErrorMessage({ error, fallback }: { error: unknown; fallback?: string }) {
  const raw = error instanceof Error ? error.message : String(error || "");
  return <div className="error-message" role="alert"><p>{humanizeError(error, fallback)}</p>{raw && <details><summary>Advanced error details</summary><pre>{raw}</pre></details>}</div>;
}
